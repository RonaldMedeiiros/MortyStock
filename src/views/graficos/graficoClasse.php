...
// Sua consulta SQL
$resolvido = "SELECT classe, COUNT(*) AS total_classes FROM produtos GROUP BY classe";
...
try {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $classe = $row['classe'];
        $total_classes = intval($row['total_classes']); // Converter para inteiro

        // Adicionar os dados formatados ao array
        $data[] = array($classe, $total_classes);
    }
  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var chart = new google.visualization.PieChart(document.getElementById('columnchart_material_entradas'));
